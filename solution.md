...
| 1 | **classify_and_extract** | `pipeline/intent.py` | Figures out what the customer wants and pulls out the relevant details. Classifies the primary (and optional secondary) intent, detects support issue status, and — for product searches — extracts structured context (activity, budget, experience, etc.) from the conversation. | `messages` (last `INTENT_CONTEXT_WINDOW=6`) | `primary_intent`, `secondary_intent`, `intent_relationship_type`, `support_status`, `intent_history`, `extracted_context`, `oos_sub_class`, `oos_complexity` | 1–2 — intent classifier (`llama3.2`) + optional context extractor (`gemma2:9b`) or OOS sub-classifier (`llama3.2`) |
| 2a | **check_completeness** (conditional edge) | `pipeline/graph.py` | Decides what happens next. If the intent isn't product search, routes straight to synthesis. If it is product search, checks whether enough context exists to make a recommendation — if not, triggers a follow-up; if yes, proceeds to translation. | `primary_intent`, `extracted_context` | nothing — routing only | 0 |
