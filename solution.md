...
| # | Name | File | What it does | Reads from state | Writes to state | LLM calls |
|---|---|---|---|---|---|---|
| 0 | **guard** (pre-graph) | `pipeline/guard.py` | Screens every user message for harmful content before the graph runs. Blocks unsafe messages with a generic rejection; safe messages pass through untouched. | — (takes raw message, not state) | — (returns response string or passes through) | 1 — `llama-guard3:latest` |
| 1 | **classify_and_extract** | `pipeline/intent.py` | Figures out what the customer wants and pulls out the relevant details. Classifies the primary (and optional secondary) intent, detects support issue status, and extracts structured context (activity, budget, experience, etc.) when product search is involved — either as the primary intent or as a compound secondary intent. | `messages` (last `INTENT_CONTEXT_WINDOW=6`) | `primary_intent`, `secondary_intent`, `intent_relationship_type`, `support_status`, `intent_history`, `extracted_context`, `oos_sub_class`, `oos_complexity` | 1–2 — intent classifier (`llama3.2`) + optional context extractor (`gemma2:9b`) or OOS sub-classifier (`llama3.2`) |
| 2a | **check_completeness** (conditional edge) | `pipeline/graph.py` | Decides what happens next. For product_search primaries: checks context completeness and routes to ask_followup or translate_specs. For compound turns (non-product primary + product_search secondary): also routes to translate_specs when context is complete, enabling the dual-pipeline path. All other intents route straight to synthesis. | `primary_intent`, `secondary_intent`, `intent_relationship_type`, `support_status`, `extracted_context` | nothing — routing only | 0 |
| 2b | **ask_followup** | `pipeline/graph.py` | Asks the customer exactly one focused question to fill the most important gap in their context (e.g. "What conditions will you be camping in?"). Waits for the next turn before proceeding. | `extracted_context`, `messages` | `response`, `messages` (appends assistant question) | 1 — `llama3.2` |
| 3 | **translate_specs** | `pipeline/translator.py` | Converts plain-language customer context into a structured product spec query. Looks up the activity in the ontology first (fast, deterministic); falls back to the LLM only when the ontology has no match. | `extracted_context` | `translated_specs` | 0–1 — ontology lookup first; LLM fallback (`gemma2:9b`) only when ontology has no match |
| 4 | **retrieve** | `pipeline/retriever.py` | Searches the product catalog using hybrid dense + sparse retrieval (RRF fusion). Returns the top-k most relevant products and a confidence signal indicating how well the catalog matched the query. | `translated_specs` | `retrieved_products`, `retrieval_confidence` | 0 — embedding only (`FastEmbed`), no LLM |
| 5 | **synthesize** | `pipeline/synthesizer.py` | Writes the final response the customer sees. Handles six cases: product recommendation (exact/close/no match), general education, support redirect, and OOS (social/benign/inappropriate). Injects safety disclaimers for high-risk activities, respects support status framing, and acknowledges secondary intents. | `primary_intent`, `secondary_intent`, `intent_relationship_type`, `support_status`, `support_handled`, `extracted_context`, `retrieved_products`, `retrieval_confidence`, `oos_sub_class`, `oos_complexity`, `intent_history`, `user_profile` | `response`, `disclaimers_applied`, `messages` (appends assistant response), `support_handled` | 1 — `gemma2:9b` (or `llama3.2` for simple/social OOS); 0 for `inappropriate` OOS (hard-coded) |

**Routing logic (check_completeness):**
- `primary_intent != product_search` + no actionable compound secondary → **synthesize**
- `primary_intent != product_search` + `secondary_intent == product_search` + `compound` + context complete + support not escalated → **translate_specs** → retrieve → synthesize *(dual-pipeline)*
...
| OOS / support / education (no compound secondary) | 2 | llama-guard3 + llama3.2 |
| Compound intent, product secondary, incomplete context | 2 | llama-guard3 + llama3.2 (intent); no extraction — falls to synthesize which invites follow-up |
| Compound intent, product secondary, complete context | 4–5 | llama-guard3 + llama3.2 (intent) + gemma2:9b (extract) + optional ontology fallback + gemma2:9b (synthesize) |
